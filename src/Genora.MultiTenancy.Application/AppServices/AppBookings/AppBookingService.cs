    ...
    IAppBookingService
{
    ...
    private readonly IRepository<CalendarSlotPrice, Guid> _calendarSlotPriceRepository;
    ...
    // Caddie repos — dùng để hủy booking Caddie liên đới khi admin đổi trạng thái booking golf sang Hủy
    ...
    public AppBookingService(
        ...
        IRepository<CalendarSlotPrice, Guid> calendarSlotPriceRepository,
        ...
        : base(repository, currentTenant, featureChecker)
    {
        ...
        _calendarSlotPriceRepository = calendarSlotPriceRepository;
    }
    ...
    public override async Task<PagedResultDto<AppBookingDto>> GetListAsync(GetBookingListInput input)
    {
        ...
        var golfCourses = golfCourseIds.Count == 0
            ? new List<GolfCourse>()
            : await _golfCourseRepository.GetListAsync(g => golfCourseIds.Contains(g.Id));
        ...
        var customerTypeIds = customers
            .Where(c => c.CustomerTypeId.HasValue && c.CustomerTypeId.Value != Guid.Empty)
        ...
        var customerTypes = customerTypeIds.Count == 0
            ...
            : await _customerType.GetListAsync(x => customerTypeIds.Contains(x.Id));

        var customerTypeDict = customerTypes.ToDictionary(x => x.Id, x => x);

        var promotionTypeIds = slots
            .Where(s => s.PromotionTypeId != Guid.Empty)
            .Select(s => s.PromotionTypeId)
            .Distinct()
            .ToList();

        var promotionTypes = promotionTypeIds.Count == 0
            ? new List<DomainModels.AppPromotionTypes.PromotionType>()
            : await _promotionTypeRepository.GetListAsync(x => promotionTypeIds.Contains(x.Id));

        var promotionTypeDict = promotionTypes.ToDictionary(x => x.Id, x => x);
        ...
        foreach (var b in items)
        {
            ...
            if (c?.CustomerTypeId.HasValue == true)
            {
                customerTypeDict.TryGetValue(c.CustomerTypeId.Value, out ct);
            }

            var ctName = ct?.Name ?? ct?.Code ?? NA();

            DomainModels.AppPromotionTypes.PromotionType? promotionType = null;
            if (slot != null && slot.PromotionTypeId != Guid.Empty)
            {
                promotionTypeDict.TryGetValue(slot.PromotionTypeId, out promotionType);
            }

            var promotionTypeName = promotionType?.Name ?? "";
            ...
            dtoList.Add(new AppBookingDto
            {
                ...
                TotalCaddieFee = b.TotalCaddieFee,
    ...
    public override async Task<AppBookingDto> GetAsync(Guid id)
    {
        ...
        var calendarSlot = booking.CalendarSlotId.HasValue && booking.CalendarSlotId.Value != Guid.Empty
            ? await _calendarSlotRepository.FindAsync(p => p.Id == booking.CalendarSlotId.Value)
            : null;
        ...
        if (calendarSlot != null && calendarSlot.PromotionTypeId != Guid.Empty)
        {
            var promotionType = await _promotionTypeRepository.FindAsync(calendarSlot.PromotionTypeId);
    ...
    public override async Task<AppBookingDto> CreateAsync(CreateUpdateAppBookingDto input)
    {
        ...
        // Phí thuê Caddie (nếu có) — cộng vào tổng tiền thanh toán booking
        entity.TotalCaddieFee = input.TotalCaddieFee;
        entity.TotalAmount += input.TotalCaddieFee ?? 0m;
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "[ZBS] Enqueue BookingCreated failed. BookingId={BookingId}, BookingCode={BookingCode}, TenantId={TenantId}",
                entity.Id,
                entity.BookingCode,
                CurrentTenant.Id
            );
        ...
        try
        {
            ...
            var golfInfo = await GetGolfCourseInfoAsync(entity.GolfCourseId);
            var promotionText = await GetPromotionNameAsync(entity.CalendarSlotId);
            var savedPlayers = await _playerRepository.GetListAsync(p => p.BookingId == entity.Id);
            ...
            {
                emailTotalAmount = entity.TotalAmount;
            }
            ...
            var model = new BookingNewRequestEmailModelDto
            {
                ...
                PricePerGolfer = entity.PricePerGolfer ?? 0m,
                PricePerGolferText = MoneyText(entity.PricePerGolfer),
    ...
    public override async Task<AppBookingDto> UpdateAsync(Guid id, CreateUpdateAppBookingDto input)
    {
        ...
        // Phí thuê Caddie (nếu có) — cộng vào tổng tiền thanh toán booking
        entity.TotalCaddieFee = input.TotalCaddieFee;
        entity.TotalAmount = input.TotalAmount + (input.TotalCaddieFee ?? 0m);
        ...
        // Nếu admin đổi trạng thái sang Hủy (hoàn tiền / không hoàn tiền) → hủy booking Caddie liên đới
        if (input.Status == BookingStatus.CancelledRefund || input.Status == BookingStatus.CancelledNoRefund)
        {
            await CancelLinkedCaddieBookingsAsync(entity.Id);
        }
        ...
        try
        {
            ...
            var oldCustomerType = oldCustomer?.CustomerTypeId.HasValue == true
                ? await _customerType.FindAsync(x => x.Id == oldCustomer.CustomerTypeId.Value)
                : null;
            var newCustomerType = newCustomer?.CustomerTypeId.HasValue == true
                ? await _customerType.FindAsync(x => x.Id == newCustomer.CustomerTypeId.Value)
                : null;

            var oldCustomerTypeText = oldCustomerType?.Name ?? oldCustomerType?.Code ?? NA();
            var newCustomerTypeText = newCustomerType?.Name ?? newCustomerType?.Code ?? NA();

            var oldPromotionText = oldSlot != null && oldSlot.PromotionTypeId != Guid.Empty
                ? (await _promotionTypeRepository.FindAsync(oldSlot.PromotionTypeId))?.Name ?? ""
                : "";

            var newPromotionText = newSlot != null && newSlot.PromotionTypeId != Guid.Empty
                ? (await _promotionTypeRepository.FindAsync(newSlot.PromotionTypeId))?.Name ?? ""
                : "";

            var oldPlayersInline = BuildPlayersInlineText(oldPlayers, oldCustomer?.FullName);
            var newPlayersInline = BuildPlayersInlineText(newPlayers, newCustomer?.FullName);

            var otherRequestsText = await BuildOtherRequestsTextAsync(entity.Utility);
            var invoiceInfoText = BuildInvoiceInfoText(entity);
            var pricePerGolferText = MoneyText(entity.PricePerGolfer);
            var totalAmountText = MoneyText(entity.TotalAmount);
            ...
            else
            {
                ...
                var changeModel = new BookingChangeRequestEmailModelDto
                {
                    ...
                    HasCaddieFee = entity.TotalCaddieFee.HasValue && entity.TotalCaddieFee.Value > 0,
                    TotalCaddieFeeText = MoneyText(entity.TotalCaddieFee ?? 0m),
                    GrandTotalText = MoneyText(emailTotalAmount + (entity.TotalCaddieFee ?? 0m)),
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "[BookingEmail] Failed to enqueue booking update/cancel email or ZBS. BookingId={BookingId}, BookingCode={BookingCode}, TenantId={TenantId}",
                entity.Id,
                entity.BookingCode,
                CurrentTenant.Id
            );
    ...
    /// <summary>
    /// Khi admin đổi trạng thái booking golf sang Hủy → hủy các AppCaddieBooking liên đới:
    /// tìm qua AppBookingPlayers.CaddieBookingId (header, distinct), set Status=Cancelled + nhả lịch Caddie.
    /// GIỮ NGUYÊN AppCaddieBookings.TotalCaddieFee (lịch sử). No-op nếu booking không có liên kết Caddie.
    /// </summary>
    private async Task CancelLinkedCaddieBookingsAsync(Guid golfBookingId)
    {
        var linkedPlayers = await _playerRepository.GetListAsync(p => p.BookingId == golfBookingId && p.CaddieBookingId != null);
        ...
        foreach (var cbId in caddieBookingIds)
        {
            ...
            if (caddieBooking == null || caddieBooking.Status == 4) continue; // 4 = Cancelled
            ...
            {
                if (d.ScheduleId == null || d.ScheduleId.Value == Guid.Empty) continue;
                try
                {
                    var schedule = await _caddieScheduleRepo.GetAsync(d.ScheduleId.Value);
                    schedule.SlotStatus = 1; // Available
            ...
            caddieBooking.Status = 4; // Cancelled — giữ nguyên TotalCaddieFee
            caddieBooking.CancelReason = "Đã hủy do hủy booking golf liên kết (admin)";
    ...
    private async Task SavePlayersAsync(Guid bookingId, List<CreateUpdateBookingPlayerDto> players)
    {
        ...
        foreach (var p in players)
        {
            ...
            // Giữ thông tin Caddie đã gán cho từng người chơi (Blue Diamond); tenant khác = null
    ...
    public async Task<IRemoteStreamContent> ExportExcelAsync(GetBookingListInput input)
    {
        ...
        var customerTypeIds = customers
            .Where(x => x.CustomerTypeId.HasValue && x.CustomerTypeId.Value != Guid.Empty)
        ...
        var promotionTypeIds = slots
            .Where(x => x.PromotionTypeId != Guid.Empty)
        ...
        string LStatus(BookingStatus s)
        {
            ...
            return (string.IsNullOrWhiteSpace(t) || t.StartsWith("BookingStatus:", StringComparison.OrdinalIgnoreCase))
                ? s.ToString()
                : t;
        }
        ...
        string LPayment(PaymentMethod? pm)
        {
            ...
            return (string.IsNullOrWhiteSpace(t) || t.StartsWith("PaymentMethod:", StringComparison.OrdinalIgnoreCase))
                ? pm.Value.ToString()
                : t;
        }
        ...
        string LSource(BookingSource src)
        {
            ...
            return (string.IsNullOrWhiteSpace(t) || t.StartsWith("BookingSource:", StringComparison.OrdinalIgnoreCase))
                ? src.ToString()
                : t;
        }
        ...
        var rows = list.Select(b =>
        {
            ...
            var customerTypeText = ct?.Name ?? ct?.Code ?? "";
            ...
            var promotionTypeText = promotionType?.Name ?? "";
            ...
            var playTime = (!string.IsNullOrWhiteSpace(from) || !string.IsNullOrWhiteSpace(to))
                ? (string.IsNullOrWhiteSpace(from)
                    ? to
                    : (string.IsNullOrWhiteSpace(to) ? from : F("Common:TimeRange", from, to)))
                : "";
            ...
            return new AppBookingExcelRowDto
            {
                ...
                PromotionType = promotionTypeText,
    ...
    private async Task<string> GetPromotionNameAsync(Guid? calendarSlotId)
    {
        ...
        var slot = await _calendarSlotRepository.FindAsync(calendarSlotId.Value);
        if (slot == null || slot.PromotionTypeId == Guid.Empty)
            return "";
    ...
    {
        var golfCourse = await _golfCourseRepository.FindAsync(golfCourseId);
        return (
    ...
    List<BookingPlayer>? players = null)
    {
        ...
        var slotPrices = await _calendarSlotPriceRepository.GetListAsync(x =>
