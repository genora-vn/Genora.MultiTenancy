    ...
    function getFilteredData() {
        ...
        if (search) {
            filtered = filtered.filter(function (item) {
                return (item.brandCode && item.brandCode.toString().includes(search)) ||
    ...
    function renderTable() {
        ...
        if (pageData.length === 0) {
            ...
            pageData.forEach(function (item) {
                ...
                tbody.append('<tr class="hl-clickable" data-code="' + (item.brandCode || '') + '"><td><code>' + (item.brandCode || '') + '</code></td><td><strong>' + (item.brandName || '') + '</strong></td><td class="text-center">' + (item.noOfProduct || 0) + '</td><td class="text-center">' + statusBadge + '</td></tr>');
            });
        }
        renderPagination();
    }
    ...
    function showDetail(brandCode) {
        ...
        Promise.all([service.getBrandDetail(brandCode.toString()), service.getProductsByBrand(brandCode.toString())])
            .then(function (results) {
                ...
                var info = '<div class="row mb-3"><div class="col-md-6"><table class="table table-sm table-borderless"><tr><td class="text-muted" style="width:120px">Mã danh mục:</td><td><strong>' + (brand ? brand.brandCode : brandCode) + '</strong></td></tr><tr><td class="text-muted">Tên:</td><td><strong>' + (brand ? brand.brandName : '-') + '</strong></td></tr><tr><td class="text-muted">Số sản phẩm:</td><td>' + (brand ? (brand.noOfProduct || 0) : products.length) + '</td></tr><tr><td class="text-muted">Trạng thái:</td><td>' + (brand && brand.isActive ? '<span class="badge hl-badge-active">Hoạt động</span>' : '<span class="badge hl-badge-inactive">Ngừng</span>') + '</td></tr></table></div></div>';
                var prodHtml = '';
                if (products.length > 0) {
                    var fc = function(v) { return v ? new Intl.NumberFormat('vi-VN').format(v) + 'đ' : ''; };
                    ...
                    products.forEach(function (p) { prodHtml += '<tr><td><code>' + (p.productGroupCode || '') + '</code></td><td>' + (p.productGroupName || '') + '</td><td>' + (p.productUnit || '') + '</td><td class="text-end">' + fc(p.productPrice) + '</td><td class="text-center">' + (p.isActive ? '<span class="badge hl-badge-active">Hoạt động</span>' : '<span class="badge hl-badge-inactive">Ngừng</span>') + '</td></tr>'; });
