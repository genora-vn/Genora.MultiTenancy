...
|-----|--------|----------|----------|
| 2026-08-26 | UrBox / Hoa Linh | **[HOTFIX 20260826]** Chi tiết đơn đổi quà Mini App (`UrBoxService.GetGiftTransactionDetailAsync`). **Fix 1:** trả về LIST `Vouchers` (map toàn bộ `cart.Detail`: Code/CodeImage/CodeDisplay/CodeDisplayType/Expired/Link/Delivery) thay vì chỉ `FirstOrDefault` → Mini App hiển thị đủ nhiều mã quà khi 1 giao dịch đổi số lượng > 1. Thêm class `UrBoxVoucherDetailDto` + prop `Vouchers` vào `UrBoxGiftTransactionDetailDto` (giữ field cũ cho voucher đầu → tương thích ngược). **Fix 2:** bỏ nhân số lượng ở `totalPoints` trong `CreateOrderEvoucherAsync` (`Sum(i => i.PointsRequired)` — FE đã nhân số lượng rồi, tránh gấp đôi); set `HlGiftExchange.PointsRequired = totalPoints` để `PointsRequired == TotalPointsUsed`. Build HttpApi 0 errors. Nhánh `hotfix/20260826`. | `UrBoxService.cs`, `UrBoxGiftTransactionDetailDto.cs` |
