...
- **Việc đang làm:** Xây hệ thống Admin cho Zalo Mini App "Dược Phẩm Hoa Linh 25 Năm" (schema DB `hl25`).
  ...
  - ✅ **P3 (Quản lý Frame) XONG:** Backend 3 AppService: `Hl25FrameCampaignAppService` (CRUD chiến dịch + đếm TemplateCount); `Hl25FrameTemplateAppService` (CRUD mẫu frame + `UploadTemplateImageAsync` 5MB, lọc theo campaign); `Hl25FrameCreationAppService` (list read-only, join Participant+Campaign). AutoMapper đủ. Trang `Web/Pages/Hl25/Frames` (3 tab: Chiến dịch DataTable+CRUD modal, Mẫu Frame DataTable + lọc chiến dịch + upload ảnh `IFormFile` server-side, Lịch sử tạo ảnh read-only) + Campaign Create/Edit + Template Create/Edit modals + `Frames.js`. Build Application + Web 0 errors. **CHƯA commit P3.**
  - ⏳ **Tiếp theo:** P5 (Người dùng) → P6 (Báo cáo) → P7 (MiniApp API).
