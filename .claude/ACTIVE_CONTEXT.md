...
- **Việc đang làm:** Xây hệ thống Admin cho Zalo Mini App "Dược Phẩm Hoa Linh 25 Năm" (schema DB `hl25`).
  ...
  - ✅ **P6 (Báo cáo Thống kê) XONG:** `Hl25ReportAppService` (3 method query AsyncExecuter, dual permission Reports): `GetFrameStatsAsync` (tổng tạo/chia sẻ + distinct người + chi tiết theo ngày), `GetWheelParticipationStatsAsync` (số người quay + tổng lượt quay/cấp + số trúng + số còn lượt), `GetWheelGiftStatsAsync` (cơ cấu giải theo quà: tổng/còn/trúng/đã trao + tỷ lệ) + DTO. Trang `Web/Pages/Hl25/Reports` (bộ lọc khoảng ngày + 3 khối báo cáo: Frame KPI+chart Chart.js line theo ngày, Vòng quay KPI, bảng theo quà) + `Reports.js`. Build Application + Web 0 errors. **CHƯA commit P6.**
  - ⏳ **Tiếp theo:** P7 (MiniApp API — controller register/frame/share/spin/gifts + spin ACID).
